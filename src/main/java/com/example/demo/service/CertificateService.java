...
//                 cert.getIssued_at(), cert.getExpire_at(), cert.getStatus(), cert.getSerial_no(), cert.getPdf_uri(), cert.getPdf_sha256()));
//     }
//     public Certificate getCertificateByCertIdAndStudentId(String certId, String studentCode) {
//         return certRepo.findByCertIdContainingAndStudentIdContaining(certId, studentCode);
//     }
//     /**
...
//                 .orElseThrow(() -> new RuntimeException("Certificate not found"));
//     }
//     public Certificate findByCertId(String certId) {
//     return certRepo.findByCertId(certId)
//             .orElseThrow(() -> new RuntimeException("Certificate not found with certId: " + certId));
// }
//     /**
...
public class CertificateService {
    ...
    public Page<CertificateDTO> getCertificatesByStudentCodePaged(String studentCode, Pageable pageable) {
        Page<Certificate> certificates = certRepo.findByStudentId(studentCode, pageable);
        return certificates.map(cert -> new CertificateDTO(cert.getId(), cert.getCertId(), cert.getStudentId(),
                cert.getIssued_at(), cert.getExpire_at(), cert.getStatus(), cert.getSerial_no(),cert.getPdf_uri(),   
        cert.getPdf_sha256()));
    }
    ...
    // }
    public Certificate getCertificateByCertIdAndStudentId(String certId, String studentCode) {
    System.out.println("=== SERVICE: Finding certificate ===");
    System.out.println("Service input - certId: '" + certId + "'");
    System.out.println("Service input - studentCode: '" + studentCode + "'");
    
    // Sử dụng Optional đúng cách
    Optional<Certificate> certificateOpt = certRepo.findByCertIdAndStudentId(certId, studentCode);
    ...
    if (certificateOpt.isPresent()) {
        ...
        System.out.println("Student ID: " + cert.getStudentId());
        System.out.println("PDF URI: " + cert.getPdf_uri());
        return cert;
    } else {
        System.out.println("=== CERTIFICATE NOT FOUND ===");
        System.out.println("No certificate found with certId: '" + certId + "' and studentCode: '" + studentCode + "'");
        
        throw new ResponseStatusException(HttpStatus.NOT_FOUND, 
            "Không tìm thấy chứng chỉ với certId: " + certId + " và studentCode: " + studentCode);
    }
}
    /**
     * Get certificate expiration statistics
