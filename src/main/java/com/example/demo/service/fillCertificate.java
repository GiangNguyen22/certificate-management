        ...
        try (InputStream is = getClass().getResourceAsStream("/templates/" + nameTemplate)) {
            ...
            try (PDDocument document = Loader.loadPDF(pdfBytes)) {
                ...
                try (InputStream fontStream = getClass().getResourceAsStream("/fonts/NotoSans-Italic.ttf");
                    ...
                    try (PDPageContentStream cs = new PDPageContentStream(document, page, PDPageContentStream.AppendMode.APPEND, true, true)) {cs.setFont(font, 14);
                        cs.beginText();
                        cs.newLineAtOffset(330, 250);
                        cs.showText(String.valueOf(today.getYear()));
                        cs.endText();
                    ...
                    try (PDPageContentStream cs = new PDPageContentStream(document, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                        cs.setFont(font, 14);
                        cs.beginText();
                        cs.newLineAtOffset(700, 173);cs.showText(String.valueOf(today.getYear()));
                        cs.endText();
                    }
