...
public class ResultService {
    ...
    public ResultService(StudentRepositoryI studentRepositoryI, CourseRepository courseRepository, ResultRepository resultRepository) {
    ...
    public Result createResult(ResultRequest resultRequest) {
        if(resultRequest.getStudentCode() == null || resultRequest.getCourseCode() == null){
         ...
         Student student = studentRepositoryI.findByStudentCode(resultRequest.getStudentCode()).orElseThrow(() -> new ResourceNotFoundEx("Student not found with code: " + resultRequest.getStudentCode()));
         Course course = courseRepository.findByCourseCode(resultRequest.getCourseCode()).orElseThrow(() -> new ResourceNotFoundEx("Course not found with code: " + resultRequest.getCourseCode()));

        if(resultRepository.existsByCourseCodeAndStudentCode(resultRequest.getCourseCode(), resultRequest.getStudentCode())){
